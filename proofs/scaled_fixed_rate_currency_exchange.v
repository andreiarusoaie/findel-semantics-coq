...
(* Scaled fixed rate currency exchange *)
Definition frce_desc (n : nat):=
  (Scale n
         (And
...
Lemma frce_execute_I_to_O :
  forall sc n I O bal time gtw ctr_id dsc_id next_id ledger result,
    execute (frce_desc n) sc I O bal time gtw ctr_id dsc_id next_id ledger =
    Some result ->
    exists tr,
...
Lemma frce_step_I_to_O:
  forall s1 s2 ctr ctr_id dsc_id I O sc n,
    step s1 s2 ->
    consistent_state s1 ->
    ctr = finctr ctr_id dsc_id (frce_desc n) I O O sc ->
